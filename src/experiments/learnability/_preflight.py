...
def main() -> None:
    ...
    print(f"  [train] seed={train_seed} n={args.train_pool_size} -> {train_d}")
    train_worlds = build_pool(config, seed=train_seed, n_levels=args.train_pool_size)
    save_pool(train_worlds, train_d)
    print(f"  [train] done")

    test_d = pool_dir(base_dir, config, "test")
    print(f"  [test]  seed={test_seed} n={args.test_pool_size} -> {test_d}")
    test_worlds = build_pool(config, seed=test_seed, n_levels=args.test_pool_size)
