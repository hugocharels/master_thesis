...
agents to exits within a bounded horizon) and the *cooperation requirement* (whether every such
trajectory must rely on the same-colour beam-blocking mechanism). Both are formally decidable and
certified by the solver for every accepted level; the SAT encoding itself is developed in
@sat-reduction. Throughout, the number of agents $n_a$ satisfies $1 <= n_a <= 4$ in line with the
LLE engine; this bound is an engine artefact, not a limitation of the encoding, which remains
...
#formalbox(kind: "definition", [Definition 3.1 (LLE Level)], [
  ...
  - $cal(E) subset P$: set of exit positions, with $|cal(E)| = |C|$.

  Here $P = {(x, y) | 0 <= x < W, 0 <= y < H}$ is the set of grid positions ($x$ is the column
...
agent of matching colour can occupy a cell of its own beam and thereby stop the beam at that cell.
The *strict* rule is a counterfactual in which lasers ignore matching agents and behave like
unblockable rays, used in @cooperation-detection to test whether a standard solution actually
relies on the blocking mechanism.

#formalbox(kind: "definition", [Definition 3.2 (Active Beams)], [
...
#formalbox(kind: "definition", [Definition 3.3 (Valid Joint Trajectory)], [
  ...
  - *No collision:* $forall t, forall c_1, c_2 in C, c_1 eq.not c_2, p_t(c_1) eq.not p_t(c_2)$.
  - *No following conflicts:* $forall t < T_("max"), forall c_1, c_2 in C, c_1 eq.not c_2$,
...
#formalbox(kind: "definition", [Definition 3.4 (Solvability)], [
...
same joint position map twice can have the intervening segment truncated without affecting
...
#formalbox(kind: "definition", [Definition 3.5 (Bounded-Horizon LLE Solvability Problem)], [
...
#formalbox(kind: "definition", [Definition 3.6 (Strict Beam Semantics)], [
  ...
  occupancy no longer blocks the corresponding beam.
]) <def-3-6>

In the model studied here, the relevant cooperative action is precisely this same-colour
beam-blocking mechanism: an agent occupies a position that would otherwise allow its own beam to
continue, thereby opening a path for another agent.

#formalbox(kind: "definition", [Definition 3.7 (Cooperation Requirement with Horizon $T_("max")$)], [
