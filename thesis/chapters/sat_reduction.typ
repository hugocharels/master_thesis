...
+ *Laser Activity*
  ...
  - *Agents cannot step on active lasers:* \
    An agent of colour $c_1$ cannot occupy a position where a laser of colour $c_2 eq.not c_1$ is
    active. Agents are immune only to lasers of their own colour:
    $
      and.big_(c_1 in C) and.big_(c_2 in C, \ c_2 eq.not c_1) and.big_((x,y) in P) and.big_(t in T)
...
- *Movement constraints.*
  ...
  The blocked-cell clauses contribute
  $
    n tau (|cal(W)| + |P_("src")|) <= n tau p
  $
...
*Proposition.* Let $L$ be an LLE level and let $T_("max")$ be a horizon. For either movement
formulation described above, the CNF formula $Phi(L, T_("max"))$ is satisfiable if and only if
there exists a valid joint trajectory of length $T_("max")$ for $L$.

*Proof sketch.* For soundness, assume $Phi(L, T_("max"))$ is satisfiable. Because initialization,
illegal-cell clauses, and the chosen uniqueness formulation constrain every agent-variable family,
for each colour $c$ and time $t$ there is exactly one position $(x, y)$ such that
$a_(c,x,y,t)$ is true. This defines a trajectory $p_t(c)$. Forward and backward consistency enforce
legal motion between consecutive steps; the collision clauses enforce both simultaneous separation
and the conservative hand-off rule; the laser clauses enforce safety with respect to active beams;
the exit clauses enforce the terminal condition. Hence the extracted trajectory is valid.
...
Set beam variables $b_(c,d,x,y,t)$ and laser variables $l_(c,x,y,t)$ according to the deterministic
beam dynamics induced by the same agent positions. Every clause family is then satisfied by
construction: initialization matches the start state, movement and collision clauses match the
trajectory semantics, beam propagation follows the induced laser dynamics, and the final positions
occupy all exits. Therefore $Phi(L, T_("max"))$ is satisfiable. $square.stroked$
